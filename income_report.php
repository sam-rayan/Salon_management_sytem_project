...
if (isset($_POST['update'])) {
    ...
    $sql_update = "UPDATE income_details SET bname = '$bname', service = '$service', money = '$money', method = '$method', dte = '$date' WHERE id = '$id'";
    if ($con->query($sql_update) === TRUE) {
        echo "<script>alert('Record updated successfully'); window.location.href='income_report.php';</script>";
    } else {
...
$sql = "SELECT * FROM income_details WHERE bname LIKE '%$searchName%'";
...
<body>
  <div class='report-container'>
    ...
    $sql_all = "SELECT * FROM income_details WHERE bname LIKE '%$searchName%'";
    ...
    if ($result_all->num_rows > 0) {
        ...
        $totalMoneyAll = 0;  // Variable to hold the total for all records

        while($row = $result_all->fetch_assoc()) {
            $totalMoneyAll += $row['money'];  // Add the money to the total
            echo "<tr>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['bname'] . "</td>
                    <td>" . $row['service'] . "</td>
                    ...
                    <td>
                        <form action='update_income.php' method='POST'>
                            ...
                            <input type='submit' value='Update' style='color:white;background-color:Blue;
                            margin-butom:0.4cm;'
                            >
                        </form>
                    </td>
        ...
        echo "</table>";
        // Display total money for all records
        echo "<div class='total'>Total Money for All Records: " . number_format($totalMoneyAll, 2) . "</div>";
    } else {
